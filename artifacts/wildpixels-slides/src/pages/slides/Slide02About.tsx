...
export default function Slide02About() {
  return (
    <div style={{ width: "100vw", height: "100vh", overflow: "hidden", position: "relative", backgroundColor: "#0A0A0A", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ position: "absolute", inset: 0, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gridTemplateRows: "1fr 1fr", gap: "3px" }}>
        {COLLAGE.map((p, i) => (
          <div key={i} style={{ position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ position: "absolute", bottom: "1vh", left: "0.8vw", fontSize: "0.55vw", color: "rgba(255,255,255,0.65)", fontFamily: "'DM Mono', monospace", letterSpacing: "0.1em", textTransform: "uppercase" }}>{p.label}</div>
          </div>
        ))}
      </div>

      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, rgba(10,10,10,0.93) 36%, rgba(10,10,10,0.45) 58%, transparent 100%)" }} />

      <div style={{ position: "absolute", top: "7vh", right: "7vw", fontSize: "0.85vw", color: "rgba(240,237,232,0.45)", fontFamily: "'DM Mono', monospace", letterSpacing: "0.12em", zIndex: 2 }}>
        Wildpixels — The Photographer
      </div>
      <div style={{ position: "absolute", bottom: "7vh", right: "7vw", fontSize: "0.9vw", color: "rgba(240,237,232,0.3)", zIndex: 2 }}>02</div>

      <div style={{ position: "absolute", top: "50%", left: "7vw", transform: "translateY(-50%)", maxWidth: "42vw", zIndex: 2 }}>
        ...
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "3.8vw", fontWeight: 700, color: "#F5F3EF", margin: 0, lineHeight: 1.1, letterSpacing: "-0.02em" }}>
          Chasing Light<br />and Wild Things
        </h2>
        <p style={{ fontSize: "1.3vw", fontWeight: 300, color: "rgba(240,237,232,0.7)", marginTop: "3vh", lineHeight: 1.75, maxWidth: "36vw" }}>
        ...
        <p style={{ fontSize: "1.3vw", fontWeight: 300, color: "rgba(240,237,232,0.7)", marginTop: "1.5vh", lineHeight: 1.75, maxWidth: "36vw" }}>
        ...
        <div style={{ marginTop: "4.5vh", display: "flex", gap: "3.5vw" }}>
          {[["18", "Places Visited"], ["1195", "Photos Taken"], ["15+", "Expeditions"]].map(([n, l]) => (
            <div key={l}>
              <div style={{ fontSize: "3vw", fontWeight: 800, color: "#F5F3EF", fontFamily: "'Playfair Display', serif", lineHeight: 1 }}>{n}</div>
              <div style={{ fontSize: "0.8vw", color: "rgba(240,237,232,0.45)", textTransform: "uppercase", letterSpacing: "0.12em", marginTop: "0.7vh" }}>{l}</div>
            </div>
          ))}
        </div>
      </div>
